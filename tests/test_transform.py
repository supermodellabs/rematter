...
def test_basic_rename(tmp_path: Path) -> None:
    ...
    status, msg = _transform_worker(
        f, from_field="created", to_field="date_created", dry_run=False
    )
    assert status == "done"
...
def test_dry_run_makes_no_changes(tmp_path: Path) -> None:
    ...
    f.write_text(original)
    status, _ = _transform_worker(
        f, from_field="created", to_field="date_created", dry_run=True
    )
...
def test_bool_value_preserved(tmp_path: Path) -> None:
    ...
    _transform_worker(
        f, from_field="is_meta_catalog", to_field="meta_catalog", dry_run=False
    )
    from rematter import _load
...
def test_cli_transform_basic(mock_source: Path) -> None:
    result = runner.invoke(
        app,
        ["transform", str(mock_source), "--field", "created", "--to", "date_created"],
    )
...
def test_cli_transform_same_field_exits_error(mock_source: Path) -> None:
    result = runner.invoke(
        app, ["transform", str(mock_source), "--field", "created", "--to", "created"]
    )
    assert result.exit_code != 0
    assert (
        "identical" in result.output.lower()
        or "identical" in (result.stderr or "").lower()
    )
...
def test_cli_transform_empty_vault(empty_vault: Path) -> None:
    result = runner.invoke(
        app,
        ["transform", str(empty_vault), "--field", "created", "--to", "date_created"],
    )
    assert result.exit_code == 0
...
def test_cli_transform_files_without_field_are_skipped(mock_source: Path) -> None:
    ...
    runner.invoke(
        app,
        ["transform", str(mock_source), "--field", "status", "--to", "reading_status"],
    )
